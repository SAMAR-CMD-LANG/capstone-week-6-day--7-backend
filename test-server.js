...
app.put("/posts/:id", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (error || !updatedPost) {
            return res.status(400).json({ message: "Not authorized to update this post or post not found" });
        }
...
app.delete("/posts/:id", authenticateToken, async (req, res) => {
    ...
    try {
        const { data: deleteData, error } = await supabase
            ...
            .single();
        if (error || !deleteData) {
            return res.status(400).json({ message: "not authorized to delete this post or post not found" });
        }
        res.status(200).json({ message: "post deleted successfully", post: deleteData });
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "internal server error" });
    }
